...
public class PauseMenuUI : UIScreen<PauseMenuUI> {
    ...
    protected override void InitializeScreen() {// Initializes the screen and determines if we are in the GameScene.
    ...
    public void AllowExit(bool allow) {
        if (allow) InputManager.Instance.PauseInputAction.performed += PauseInputActionOnPerformed;
        else InputManager.Instance.PauseInputAction.performed -= PauseInputActionOnPerformed;
    ...
    private IEnumerator LogoutThenQuit(bool headToMenuScene) {
        ...
        if (headToMenuScene) yield return SceneManager.LoadSceneAsync("mains_creen");// Load the main menu scene
        else Application.Quit();// Quit the game application
    }
    
    // Opens the pause menu screen
    public override void OpenScreen() {
        if (_isGameScene && UIManager.Instance.ActivityState) return; // Prevent opening if the game is active (e.g. cutscene, animation)
