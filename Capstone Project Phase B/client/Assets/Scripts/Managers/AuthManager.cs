...
public class AuthManager : SuperBehaviour {// Manages login, registration, and secure credential handling
    ...
    [SerializeField] private TMP_InputField loginUsernameInput;// Input field for username during login
    ...
    [SerializeField] private TMP_InputField registerEmailInput;// Input field for email during registration
    ...
    private void LoadSavedCredentials() { // Attempts to load previously saved credentials from PlayerPrefs
        ...
        try {
            string decryptedPassword = EncryptionHelper.Decrypt(encryptedPassword, savedUsernameTemp);// Attempt decryption
    ...
    private IEnumerator Login(string username, string password) {// Construct login URL
       ...
       // Stop animation
        ...
        } else { // Parse and process successful login
            LoginResponse loginResponse = JsonUtility.FromJson<LoginResponse>(request.downloadHandler.text);
