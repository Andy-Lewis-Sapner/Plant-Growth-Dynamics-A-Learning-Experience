...
public class HouseEnvironment : Singleton<HouseEnvironment>, IEnvironment {
    public event EventHandler OnStateChanged;// Event triggered when the environment state changes (lights or air conditioners)
    
    private const float InsulationFactor = 0.5f;  // How much the house insulates against outside temperature (0 = full insulation, 1 = no insulation)
    ...
    public bool AreLightsOn {   // Whether the house lights are turned on
        get => _areLightsOn;
        set {
    ...
    public bool AreAirConditionersOpen {// Whether the air conditioners are turned on
    ...
    public void TurnAllHouseLights(bool turnOn) {// Turns all house lights on or off
        if (AreLightsOn == turnOn) return;
        AreLightsOn = turnOn;
    }
    
    public void OpenAllAirConditioners(bool open) { // Turns all air conditioners on or off
    ...
    public float GetHumidity(float humidity) {  // Calculates adjusted humidity inside the house based on outdoor humidity and AC state
    ...
    public float GetTemperature(float outdoorTemp) {  // Calculates indoor temperature based on outdoor temperature, insulation, and AC state
