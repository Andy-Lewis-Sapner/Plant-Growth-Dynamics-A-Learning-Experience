...
public class GroundEnvironment : Singleton<GroundEnvironment>, IEnvironment {
    ...
    public bool AreLightsOn {   // Whether the street lights are currently on
        get => _areLightsOn;
        set {
    ...
    public bool AreSprinklersOpen {// Whether the sprinklers are currently open
    ...
    public void OpenAllSprinklers(bool open) { // Opens or closes all sprinklers based on the given flag
    ...
    public void TurnStreetLights(bool turnOn) {// Turns the street lights on or off
        if (AreLightsOn == turnOn) return;
        AreLightsOn = turnOn;
    }

    public static float GetHumidity() { // Returns the current humidity from the weather manager
        return WeatherManager.Instance.OpenMeteoHour.relative_humidity_2m;
    }

    public static float GetTemperature() { // Returns the current temperature from the weather manager
        return WeatherManager.Instance.OpenMeteoHour.temperature_2m;
    }

    public static float GetLightLevel() { // Calculates the total light level based on direct and diffuse radiation
