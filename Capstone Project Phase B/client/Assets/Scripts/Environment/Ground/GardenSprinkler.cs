...
public class GardenSprinkler : SuperBehaviour, IUpdateObserver {
    ...
    private readonly Vector3 _closedPosition = new(0, -0.18f, 0);  // Local position when the sprinkler is retracted/closed
    ...
    private void Awake() {// Called when the object is initialized
        waterParticles?.Stop(); // Ensure water is not playing at start
    }

    private void Start() { // Called on the first frame – sets initial state
        sprinklerClosed.SetActive(true);
        sprinklerOpened.SetActive(false);
        _isOpen = false;
    }
    
    public void ObservedUpdate() {// Called every frame while the sprinkler is open (when registered)
        if (_isOpen)
            sprinklerOpened.transform.Rotate(Vector3.forward, RotationSpeed * Time.deltaTime);
    }

    public void OpenSprinkler() {// Opens the sprinkler, plays animation and water effect
        if (_isOpen) return;
        _isOpen = true;
    ...
    public void CloseSprinkler() {// Closes the sprinkler, stops animation and water effect
        if (!_isOpen) return;
        _isOpen = false;
