...
public class FanRotation : SuperBehaviour, IUpdateObserver {
    ...
    private const float
        MaxUpdateDistanceSqr =
            2500f; // Maximum squared distance from the player for the fan to update (performance optimization)
    ...
    public void ObservedUpdate() {
        // Rotates the fan based on the current state, player proximity, and other factors
    ...
    private void OnEnable() =>
        UpdateManager.RegisterObserver(this); // Register this object with the UpdateManager when enabled

    private void OnDisable() =>
        UpdateManager.UnregisterObserver(this); // Unregister this object from the UpdateManager when disabled
}