...
public class FlightSequence : SuperBehaviour, IUpdateObserver {
    private const float
        DistanceToStartFlightSqr =
            25f; // Distance (squared) from the spaceship to the player required to start the flight
    ...
    public void ObservedUpdate() {
        // Called every frame by the UpdateManager if this observer is registered
        if (MissionManager.Instance.CurrentCheckpoint != MissionManager.Instance.TotalCheckpoints - 1)
            return; // Only trigger if the player has reached the last checkpoint
        float distanceFromPlayer =
            (spaceship.transform.position - Player.Instance.PlayerPosition)
            .sqrMagnitude; // Check if the player is close enough to the spaceship
    ...
    private IEnumerator ContinueToGame() {
        // Fades to black and transitions to the main game
    ...
    private void OnDestroy() {
        // Stop coroutine on destruction to avoid memory leaks
