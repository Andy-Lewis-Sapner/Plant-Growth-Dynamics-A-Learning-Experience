        ...
        public void InvokeMethod(string methodName) 
        {
            ...
            foreach (UnityEngine.Object obj in m_methods.Keys)
            {
                foreach (MethodInfo method in m_methods[obj])
                {
                    ...
                    {
                        Debug.LogWarning($"Failed to invoke method {methodName} on {obj.name}. Error: {e.Message}");
                    }
                }
            }
        }
        ...
        public T InvokeMethod<T>(string methodName, params object[] parameters) 
        {
            foreach (UnityEngine.Object obj in m_methods.Keys)
            {
                foreach (MethodInfo method in m_methods[obj])
                {
                    ...
                    {
                        Debug.LogWarning($"Failed to invoke method {methodName} on {obj.name} with parameters {parameters}. Error: {e.Message}");
                    }
                }
            }
            return default;
